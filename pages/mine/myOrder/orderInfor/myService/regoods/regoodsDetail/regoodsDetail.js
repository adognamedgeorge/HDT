...
Component({
  ...
  methods: {
    ...
    //修改售后申请页面
    modifyReturnRequest (event) {
      let that = this;
      wx.navigateTo({
        url: '../regoods?pk='+that.data.orderEntryPk+"&code="+that.data.code,
      })
    },

    //撤销售后申请
    cancelReturnRequest (event) {
      let that = this;
      user.checkLogin().then((function (res) {
        if (res) {
    ...
    toRegoodsLog () {
      let that = this;
      wx.navigateTo({
        url: './regoodsLog/regoodsLog?code='+that.data.code
      })
    }
  }
});
